fcn_simul_VARMA11_LTY_2values_phiphihat<- function(scaleVset, option , KFfit, MM,TT,phiset,phihatset,simulnumber   ) {
  
  filename = fcn_filename_maker(scaleVset=scaleVset, option=option )
  load(file=filename)
   
  erphi    = c(rep(phiset[1], TT/2), rep(phiset[2], TT/2))
  erphihat =  c(rep(phihatset[1], TT/2),rep(phihatset[2], TT/2))
  ...
  for (iii in seq(1,MM)){
    ...
    ytsim2 = ytsim1   
        ...
        for (ii  in seq(2, TT ))
        {           
          xtsim1     =   erphi[ii]*xtsim0 + et_sim[ii] 
          ytsim1[ii] = KFfit$par["mue"] + xtsim1 + at_sim[ii] 
          ytsim2[ii] = KFfit$par["muehat"] + erphihat[ii]*(KFfit$par["Khat"]*ytsim1[ii-1] +
   ...
   yt = matrix(rbind(ytsim1,ytsim2),nrow=2) 
    ...
    filename = sprintf("theta_sim_VARMA_MLE_2values_phiphihat_%d.Rdata", simulnumber ) 
