fcn_VARMA_MLE_table_general  <- function( filename, fixedeffect,simulnumber) {  
...
for (jjj in seq(1,simulnumber))  {
  load( sprintf("theta_sim_VARMA_MLE_%s%s_%d.Rdata",filename,temp,jjj)) 
  theta_sim_all = cbind(theta_sim_all,theta_sim)
}
  
apply(theta_sim,1,median,na.rm=T)
apply(theta_sim,1,sd,na.rm=T)
...
ps1=rbind(ps1,ps)
  ...
  write.csv(x = sample.dataframe, file = sprintf("Table4%s_%s.csv",temp,filename ))
