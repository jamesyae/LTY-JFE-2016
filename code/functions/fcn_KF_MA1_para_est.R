fcn_KF_MA1_para_est <- function(y) {  
  # Univariate KF
  
  
  arma21ss <- function( ct, ma1, sigma) {
    Tt <- matrix(c(0, 0, ma1, 0), ncol = 2)
  ...
  KFobjective <- function(theta, yt) {  
    sp <- arma21ss(theta["ct"], theta["ma1"], theta["sigma"]) 
  ...
  theta <- list( ct = mean(y,na.rm=T),  ma1 =  cor(y[1:(length(y)-1)],y[2:length(y)], use="complete.obs")
