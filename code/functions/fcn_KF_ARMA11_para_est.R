fcn_KF_ARMA11_para_est <- function(y) {  
  # Univariate KF
  
  
  arma21ss <- function( ct,ar1, ma1, sigma) {
    Tt <- matrix(c(ar1, 0, ma1, 0), ncol = 2)
  ...
  KFobjective <- function(theta, yt) {  
    sp <- arma21ss(theta["ct"],theta["ar1"],theta["ma1"], theta["sigma"]) 
