fcn_simul_ARIMA_firmlevel <- function(scaleVset, option , savefilename, arcoef_set,  sigmaa2_sigmae2_set, KK) {
  ...
  for (kk in seq(1,KK)) {
    ...
    for (mm in seq(1,MM)) {
      ...
      while (jj_start <=NN  &  is.na(artemp[kk*NN]+matemp[kk*NN]+ar1temp[kk*NN])   ) 
      {
        ...
        {  
          ...
          tryCatch({
            ...
            m1=fcn_KF_AR1_para_est(y1)
            m2=fcn_KF_ARMA11_para_est(y1)

            
          }, error=function(e){cat("ERROR :",conditionMessage(e), "\n")
                               erroroccur = TRUE} )
            
          reverse_idx[(kk-1)*NN+jj,mm] = m1$loglkhd > m2$loglkhd
          ar1temp[(kk-1)*NN+jj,mm] = m1$par[2]              
          artemp[(kk-1)*NN+jj,mm] = m2$par[2]
          matemp[(kk-1)*NN+jj,mm] = m2$par[3] 
      ...
      firmsimul = list(ar1temp=ar1temp,artemp=artemp,matemp=matemp,arcoef_set=arcoef_set, 
                       reverse_idx=reverse_idx, sigmaa2_sigmae2_set=sigmaa2_sigmae2_set) 
