fcn_KF_VARMA11_LTY_est_restricted <- function(yt, mueest) {  
# Univariate KF
...
Varma11_LTY_trans <- function(theta) {
    ...
    ct = matrix(as.numeric(c( mue , theta["muehat"] )),2,1) 
    Zt = matrix(c(1,0,0,0,0,1),2,3) 
    Tt = matrix(c(as.numeric(phi),0,as.numeric(theta["phihat"])*as.numeric(theta["Khat"]),
                  1,0,0,0,0,as.numeric(theta["phihat"])*(1-as.numeric(theta["Khat"]))),3,3)   
    
    q = matrix(c(1, -as.numeric(phi), 0, 1 ,0 ,as.numeric(theta["what"]),0,0,as.numeric(theta["what"]) ),3,3)   
    HHt = q%*%diag(exp(2*c(as.numeric(c(theta["logsigmaa"], logsigmae, theta["logsigman"])))))%*%t(q)    
...
phi = as.numeric(m1$coef[1])
mue = as.numeric(m1$coef[2])
logsigmae=  as.numeric(log(sqrt( m1$sigma2   )))
...
KFfit$parR = list(mue=mue,muehat=KFfit$par["muehat"],phi=phi,phihat=KFfit$par["phihat"],
                  Khat=KFfit$par["Khat"],what=KFfit$par["what"],
                  logsigmaa=KFfit$par["logsigmaa"],logsigmae=logsigmae,
                  logsigman=KFfit$par["logsigman"]       )
