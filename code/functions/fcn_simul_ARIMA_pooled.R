fcn_simul_ARIMA_pooled  <- function(modelorder, scaleVset, option  , fixedeffect , theta_start, MM){  
  

theta_sim = matrix(rep(NA,2*MM),nrow=2)

for (iii in seq(1,MM)){   
  yyy1 = fcn_arima_LTY_random_data( scaleVset=scaleVset, option=option  , fixedeffect=fixedeffect , theta_start  )  
...
filename = sprintf("theta_sim_AR%dMA%d%s.Rdata",modelorder[1],modelorder[3],temp) 
