...
load("fcn_firmlevel_TS_make_general.Rdata")
    ...
    for (ii in c( (2*window_size-1):max_no_obs_deflated)) {
      if (est$no_obs_deflated[jj] >= ii ) {
        ...
        #tryCatch({
          ...
          if (ii==2*window_size-1) FErhoadj_firmqtr_20_reg[jj,  yearqtr_idx_temp ]  = FErho_learn_0_reg[jj,ii-(2*window_size-2)] 
          if (ii==3*window_size-1) FErhoadj_firmqtr_30_reg[jj,  yearqtr_idx_temp ]  = FErho_learn_0_reg[jj,ii-(3*window_size-2)]
          if (ii==4*window_size-1) FErhoadj_firmqtr_40_reg[jj,  yearqtr_idx_temp ]  = FErho_learn_0_reg[jj,ii-(4*window_size-2)]
          if (ii==5*window_size-1) FErhoadj_firmqtr_50_reg[jj,  yearqtr_idx_temp ]  = FErho_learn_0_reg[jj,ii-(5*window_size-2)]
...
mm0  =(lm( c(t(fcn_1_1_winsor(FErhoadj_firmqtr_all))) ~  factor(t( matrix(year_dummy.panel,ncol=dim(FErhoadj_firmqtr_all)[2] ) )) + c(t(age_proxy$trueyearqtr_idx_numeric.panel*4))       ))
summary(mm0)
sqrt(diag(NeweyWest(mm0 )))

mm0  =(lm( c(t(fcn_1_1_winsor(FErhoadj_firmqtr_all))) ~  factor(t( matrix(year_dummy.panel,ncol=dim(FErhoadj_firmqtr_all)[2] ) )) + c(t(age_proxy$nonNA_index_firm.panel))       ))
summary(mm0)
sqrt(diag(NeweyWest(mm0 )))
...
plot(yearqtr_xaxis,apply(fcn_1_1_winsor(FErhoadj_firmqtr_20_reg[idxtemp,]) ,2,mean, na.rm=T), type="o",ylim=c( 0.1,0.25))
lines(yearqtr_xaxis,apply(fcn_1_1_winsor(FErhoadj_firmqtr_all[idxtemp,]) ,2,mean, na.rm=T), lty=3,ylim=c( 0.1,0.25))
lines(yearqtr_xaxis,apply(fcn_1_1_winsor(FErhoadj_firmqtr_40_reg[idxtemp,]) ,2,mean, na.rm=T), lty=3,ylim=c( 0.1,0.25))
lines(yearqtr_xaxis,apply(fcn_1_1_winsor(FErhoadj_firmqtr_50_reg[idxtemp,]) ,2,mean, na.rm=T), lty=3,ylim=c( 0.1,0.25))


x = fcn_1_1_winsor(FErhoadj_firmqtr_all)
x[age_proxy$trueyearqtr_idx_numeric.panel> 10] = NA
plot(  yearqtr_xaxis,apply(x,2,mean,na.rm=T) ,type="o" )
lines(yearqtr_xaxis,apply(fcn_1_1_winsor(FErhoadj_firmqtr_20_reg[idxtemp,]) ,2,mean, na.rm=T), lty=3 )
...
load("phi_learn_lmer00_cal_fill_all.Rdata")
plot(yearqtr_xaxis,phi_learn$phiphihatvar, type="o")
load("phi_learn_lmer00_cal_fill_old.Rdata")
lines(yearqtr_xaxis,phi_learn$phiphihatvar, lty=2)
a=phi_learn$phiphihatvar
load("phi_learn_lmer00_cal_fill_young.Rdata")
lines(yearqtr_xaxis,phi_learn$phiphihatvar-a , type="o")
plot(yearqtr_xaxis,phi_learn$phiphihatvar-a , type="o")
...
plot(yearqtr_xaxis,apply( fcn_1_1_winsor( FErhoadj_firmqtr_all.tempyoung[idxtemp,]) ,2,mean, na.rm=T)-apply( fcn_1_1_winsor(  FErhoadj_firmqtr_all.tempold[idxtemp,]) ,2,mean, na.rm=T), type="o",ylim=c( 0 ,0.125))
...
### avg of quarter-by-quarer age-quantile groups altogther
idxtemp =   !firm_at_1985_5_exclude_index  
no_group =5
qgroup_means_all = rep(NA, no_group)
x = fcn_1_1_winsor(FErhoadj_firmqtr_all[idxtemp,])-
     t(matrix(rep(apply(fcn_1_1_winsor(FErhoadj_firmqtr_all[idxtemp,]),2,mean,na.rm=T), sum(idxtemp,na.rm=T)),nrow=length(yearqtr_xaxis)))
        
for (ii in c(1:no_group)) {
  
  quantile_group_idx = matrix(rep(NA,  length(yearqtr_xaxis)*sum(idxtemp,na.rm=T)  ), ncol=  length(yearqtr_xaxis))
  ...
  for (jj in seq(  1 , length(yearqtr_xaxis) ) ){     
    ...
    quantile_group_idx[,jj] = break_points[ii] <= crt   &  crt  <break_points[ii+1] 
  }
   
    
  qgroup_means_all[ii] = mean(x[quantile_group_idx] ,na.rm=T)
  
}
barplot( qgroup_means_all )




### avg of whole age-quantile groups altogther
idxtemp =   !firm_at_1985_5_exclude_index*0   
no_group =5
x = fcn_1_1_winsor(FErhoadj_firmqtr_all[idxtemp,] ) -
   t(matrix(rep(apply(fcn_1_1_winsor(FErhoadj_firmqtr_all[idxtemp,] ),2,mean,na.rm=T), sum(idxtemp,na.rm=T)),nrow=length(yearqtr_xaxis)))
 
crt=age_proxy$trueyearqtr_idx_numeric.panel[idxtemp,] 
    crt = age_proxy$nonNA_index_firm.panel[idxtemp,] 
  #crt = age_proxy$TS_index_firm_all.panel[idxtemp,] 
qgroup_means_all = rep(NA, no_group)
break_points = quantile(crt, seq(0, 1, by=1/no_group),na.rm=T)
break_points[no_group+1]= Inf 
for (ii in c(1:no_group)) {
...
barplot( qgroup_means_all,ylim=c(-0.02,0.02))
...
range_nonNA_jj = rep(NA, dim(FErhoadj_firmqtr_all)[1])
no_nonNA_jj = range_nonNA_jj
for (jj in  c(1:dim(FErhoadj_firmqtr_all)[1]  ) ) {
  if (sum( !is.na(FErhoadj_firmqtr_all[jj,]))>0) {
    range_nonNA_jj[jj] = which(!is.na(FErhoadj_firmqtr_all[jj,]))[ sum(!is.na(FErhoadj_firmqtr_all[jj,])) ]   -  which(!is.na(FErhoadj_firmqtr_all[jj,]))[1]  +1
    no_nonNA_jj[jj] = sum(!is.na(FErhoadj_firmqtr_all[jj,]))
  }
}

hist(no_nonNA_jj/range_nonNA_jj,10)
mean(range_nonNA_jj-no_nonNA_jj,na.rm=T)
mean((range_nonNA_jj-no_nonNA_jj)/no_nonNA_jj,na.rm=T)
...
if (do_all) {
  ...
  {  
    ...
    for (ii in c( (onetoT[nonNA_index_firm_temp==window_size] + 1):length(yearqtr_xaxis)  ) ) {
      
      nonNA_index_firm_temp_left = nonNA_index_firm_temp[1:(ii-1)]
      nonNA_index_firm_temp_right = nonNA_index_firm_temp[(ii+1):length(nonNA_index_firm_temp)]
      ...
      if (no_nonNA_left >= window_size & no_nonNA_right >= window_size ) {
        
        withinfirm_idx_temp = seq(onetoT[nonNA_index_firm_temp==(max_no_nonNA_left-window_size+1)],onetoT[nonNA_index_firm_temp==(min_no_nonNA_left +window_size-1 )])
        xxx0 = xxx[ withinfirm_idx_temp ]          
        xxx0_0 = c(NA, xxx0[1:(length(xxx0)-1)])
        xxx0_1 = xxx0 
        
        yearqtr_idx_one_temp = yearqtr_idx_one[ firm_idx_temp[ ii ]   ]
        FErhoadj_firmqtr_all[jj,  yearqtr_idx_one_temp ] = (lm(xxx0_1 ~ xxx0_0))$coef[2] 
  ...
  FErhoadj_firmqtr_all =  (FErhoadj_firmqtr_all *( (2*window_size-1) - 1)+1) / ( (2*window_size-1) - 4)      
  save(FErhoadj_firmqtr_all,   file="FErhoadj_firmqtr_all_fill_center.Rdata")
...
if (do_all) {
  ...
  {  
    ...
    for (ii in c( (onetoT[nonNA_index_firm_temp==window_size] + 1):length(yearqtr_xaxis)  ) ) {
      ...
      nonNA_index_firm_temp_left = nonNA_index_firm_temp[1:(ii-1)]
      nonNA_index_firm_temp_right = nonNA_index_firm_temp[ii:length(nonNA_index_firm_temp)]
      ...
      if (no_nonNA_left >= window_size & no_nonNA_right >= window_size ) {
        ...
        TS_index_firm_all.panel[jj, yearqtr_idx_one_temp ] = TS_index_firm_all[  firm_idx_temp[ii] ] 
        nonNA_index_firm.panel[jj, yearqtr_idx_one_temp] = max( no_nonNA_left, nonNA_index_firm[ firm_idx_temp[ii]] ) 
        trueyearqtr_idx_numeric.panel[jj, yearqtr_idx_one_temp ] =  trueyearqtr_idx_numeric[firm_idx_temp[ii]]
  ...
  age_proxy = list(TS_index_firm_all.panel=TS_index_firm_all.panel, nonNA_index_firm.panel=nonNA_index_firm.panel,
                   trueyearqtr_idx_numeric.panel=trueyearqtr_idx_numeric.panel )
  save(age_proxy, file="age_proxy_fill.Rdata") 
...
if (do_all) {
  ...
  window_size = 10
  no_window_learn = length(yearqtr_xaxis)
  ...
  for (ii in c( (window_size+1):(length(yearqtr_xaxis)-window_size)   )) {    
    ...
    for (jj in seq(  1 , length(est$firm_begin_deflated ) ) )   {  
      ...
      if (length(kk)==1){
        ...
        if ( nonNA_index_firm_temp[kk] > 0  & no_nonNA_left>=window_size   & no_nonNA_right>=window_size ){
          
          
          kk1 = which(nonNA_index_firm_temp == nonNA_index_firm_temp[kk]-window_size)
          kk2 = which(nonNA_index_firm_temp == nonNA_index_firm_temp[kk]+window_size)
          
          total_idx_temp[firm_idx_temp[ kk1:kk2]   ]=TRUE     
          if ( no_nonNA_left == window_size )  total_idx_temp_20[firm_idx_temp[ kk1:kk2]   ]=TRUE     
          if ( no_nonNA_left == window_size*2 ) total_idx_temp_10_30[ firm_idx_temp[ kk1:kk2] ]=TRUE             
        }   
    ...
    tryCatch({
      ...
      M0.phiphihat = lmer(FE_temp ~ ye_temp_lag1+ (1| firm_dummy ) + (-1 + ye_temp_lag1 | firm_dummy )   ,subset = total_idx_temp_20,  REML=TRUE)      
      phiphihat_learn_lmer_var00_20[ii] =  (attr( VarCorr(M0.phiphihat)$firm_dummy.1 ,"stddev")[1])^2
      
      if ( sum( total_idx_temp_10_30 ) >0 ) {
        M0.phiphihat = lmer(FE_temp ~ ye_temp_lag1+ (1| firm_dummy ) + (-1 + ye_temp_lag1 | firm_dummy )   ,subset = total_idx_temp_10_30,  REML=TRUE)
        phiphihat_learn_lmer_var00_10_30[ii] =  (attr( VarCorr(M0.phiphihat)$firm_dummy.1 ,"stddev")[1])^2
      }
  ...
  save(phi_learn, file="phi_learn_lmer00_center.Rdata") 
  
}


load("phi_learn_lmer00_center.Rdata")  
load("FErhoadj_firmqtr_all_center.Rdata")
sum(  is.na(phi_learn$phiphihatvar) !=  is.na(FErhoadj_firmqtr_all)   )

plot(yearqtr_xaxis, phi_learn$phiphihatvar ,ylim=c(0,0.015),type="o")
plot(yearqtr_xaxis, phi_learn$phiphihatvar_20 ,ylim=c(0,0.015),type="o")
