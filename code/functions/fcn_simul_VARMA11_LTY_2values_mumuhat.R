fcn_simul_VARMA11_LTY_2values_mumuhat<- function(scaleVset, option , KFfit, MM,TT,muset,muhatset,simulnumber   ) {
  
  filename = fcn_filename_maker(scaleVset=scaleVset, option=option )
  load(file=filename)
   
  ermu    = c(rep(muset[1], TT/2), rep(muset[2], TT/2))
  ermuhat =  c(rep(muhatset[1], TT/2),rep(muhatset[2], TT/2))
  ...
  for (iii in seq(1,MM)){
    ...
    ytsim2 = ytsim1   
        ...
        {           
          ...
          ytsim1[ii] = ermu[ii] + xtsim1 + at_sim[ii] 
          ytsim2[ii] = ermuhat[ii]*(1-KFfit$par["phihat"]) + KFfit$par["phihat"]*(KFfit$par["Khat"]*ytsim1[ii-1] +
   ...
   yt = matrix(rbind(ytsim1,ytsim2),nrow=2) 
    ...
    filename = sprintf("theta_sim_VARMA_MLE_2values_mumuhat_%d.Rdata", simulnumber ) 
